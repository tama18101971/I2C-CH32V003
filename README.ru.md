...
Версия 6.0.0 обеспечивает **сокращение Flash-памяти примерно на 25%** (см. [CHANGELOG](CHANGELOG.md))
поверх драйвера, уже защищённого от сбоев шины и глухих зависаний:
...
4. **Абсолютная защита бита ACK при авариях:** Во всех ветках программных таймаутов функций чтения (`i2c_read_register` и `i2c_read_buffer` для пакетов любой длины: `len==1`, `len==2`, `len>=3`) перед выходом принудительно восстанавливается бит `ACK = 1`. Это гарантирует, что упавшая посреди чтения транзакция не сломает последующие вызовы шины.
...
10. **Единый автомат ожидания флага STAR1 (v6.0.0):** Внутренний хелпер `i2c_wait_star1_flag()`
    централизует опрос флагов, обработку `BERR`/`ARLO`, `AF` и таймаута с восстановлением во всех
    wait-циклах, устраняя ~250–350 B дублированного машинного кода.
11. **Оптимизированные задержки и настройка тактирования (v6.0.0):** `i2c_usleep()`/`i2c_delay()`
    используют регистровые `nop`-циклы без стековых счётчиков и 32-битного деления в рантайме
    (частота берётся из настроенного регистра `I2C1->CTLR2`); `i2c_configure_registers()`
    вычисляет делитель CCR в едином пути.
12. **Компактное управление GPIO и единая сборка (v6.0.0):** Хелпер `i2c_set_gpio_mode()`
    устраняет повторные read-modify-write операции над `GPIOC->CFGLR`, а корневой `platformio.ini`
    собирает любой пример/бенчмарк одной командой.
...
* `uint8_t i2c_init(uint32_t bound);`
  Выполняет сброс блока I2C1 через `SWRST`, настраивает тактирование GPIO и периферии, вычисляет значения регистров `CTLR2` и `CKCFGR` для работы в Standard Mode (до 100 кГц) или Fast Mode (до 400 кГц) на основе текущей частоты `SystemCoreClock`. Автоматически выставляет обязательный 14-й бит в `OADDR1`. Возвращает `I2C_OK` или `I2C_ERR_CLK` если `SystemCoreClock` находится за пределами допустимого диапазона 2..48 МГц.
...
* `uint8_t i2c_wait_bus_free(void);`
  Опрашивает флаг `BUSY`. Также определяет аппаратные ошибки `BERR`/`ARLO` внутри цикла для немедленного восстановления. Если флаг не сбросился за время `I2C_TIMEOUT`, функция аварийно вызывает `i2c_bus_recovery`. Возвращает `I2C_OK` или `I2C_NACK`.
...
* `uint8_t i2c_stop(void);`
  Выставляет бит `STOP`. Best-effort: возвращает `I2C_OK` или `I2C_NACK` если шина не освободилась. При неудаче автоматически запускает восстановление шины.
* `uint8_t i2c_probe_address(uint8_t addr, uint16_t *p_star1, uint16_t *p_star2);`
  Проверка одного 7-битного I2C-адреса. Возвращает `I2C_OK` если устройство ответило ACK, иначе `I2C_NACK`. По желанию сохраняет значения регистров `STAR1`/`STAR2` для диагностики (передайте `NULL` чтобы пропустить). Генерирует ровно один STOP и при успехе, и при ошибке.
...
Высокоуровневая функция `i2c_probe_address()` инкапсулирует START, отправку адреса и STOP в один безопасный вызов — ручное низкоуровневое управление не требуется. Вызов адреса, на который никто не отвечает, возвращает обычный `I2C_NACK`, не приводя к сбросу шины или зависанию драйвера.

> **Примечание:** Пример использует `printf()` для вывода в консоль. На CH32V003 для этого необходимо самостоятельно перенаправить вывод стандартного потока на UART (реализовать функцию `_write`/`putchar` с отправкой байт через USART). Без такого retarget'а вызовы `printf` не дадут видимого результата.
...
void i2c_scan_bus(void) {
    ...
    for (uint8_t i = 0; i < 128; i += 16) {
        ...
        for (uint8_t j = 0; j < 16; j++) {
            ...
            // i2c_probe_address() сам выполняет START, адрес и STOP
...
В датчиках с внутренним цикличным FIFO-буфером критически важно вычитать ровно столько байт, сколько указал датчик. Лишний незапланированный импульс SCL приведет к тому, что датчик безвозвратно удалит следующий байт из своей памяти. Наш драйвер выполняет чтение N ≥ 3 строго по спецификации.
...
#### Преимущество драйвера для ЦАП (отказоустойчивость):
...
4. Силами GPIO формируется валидная последовательность STOP-условия: перевод SCL в LOW → перевод SDA в LOW → перевод SCL в HIGH → перевод SDA в HIGH.
...
7. Вызывается функция восстановления конфигурационных регистров шины. Восстанавливаются параметры тактирования и повторно активируется аппаратный контроль бита ACK.
...
Полный рабочий пример сканирования шины I2C с готовым `platformio.ini` находится в
[`examples/i2c_scanner`](examples/i2c_scanner). Минимальный бенчмарк размера Flash — в
[`examples/size_benchmark`](examples/size_benchmark).

В репозитории также есть **единый корневой `platformio.ini`** — любой пример или бенчмарк
можно собрать прямо из корня одной командой:

```sh
pio run -e scanner        # Сканер шины I2C
pio run -e benchmark_full # Бенчмарк размера Flash (Full-сборка)
pio run -e benchmark_lite # Бенчмарк размера Flash (Lite-сборка)
```
...
Для приложений с жёсткими ограничениями по размеру Flash (16 КБ у CH32V003) реализован **Aggressive Lite**-режим. Он отключает часть кода через препроцессорные макросы — таймаут во всех операциях сохранён, но при аппаратной ошибке (`BERR`/`ARLO`) или зависании шины функции просто возвращают `I2C_NACK` без попытки аппаратного восстановления шины.
...
Или точечно отключайте только нужные блоки:
```ini
build_flags =
    -DI2C_DISABLE_BUS_RECOVERY    ; убрать i2c_bus_recovery()
    -DI2C_DISABLE_SCANNER          ; убрать i2c_probe_address()
    -DI2C_DISABLE_BUFFER_API      ; убрать i2c_write_buffer()/i2c_read_buffer()
    -DI2C_DISABLE_ERROR_COUNTER   ; убрать consecutive_errors + handle_critical_error()
```

Макрос `I2C_LITE=1` эквивалентен включению всех четырёх `I2C_DISABLE_*` одновременно. По умолчанию (макрос не задан или `I2C_LITE=0`) собирается полная отказоустойчивая версия.
...
| Счётчик `consecutive_errors` + `handle_critical_error` | **−** | + |

### Поведение при ошибках в Lite-режиме

Все wait-циклы остаются ограничены `I2C_TIMEOUT` — драйвер **никогда не зависает намертво** (в отличие от WCH EVT). При `BERR`/`ARLO` или таймауте функция:
1. Сбрасывает флаг ошибки.
2. Пытается выдать `i2c_stop()` для освобождения шины (там, где это уместно).
3. Возвращает `I2C_NACK`.

Если внешнее устройство удерживает SDA в LOW после сбоя, шина останется в состоянии `BUSY` до следующей перезагрузки периферии (`i2c_deinit()`/`i2c_init()` или сброс питания). Аппаратное восстановление (16 тактов SCL через GPIO) доступно только в полной версии.

### Измеренная экономия Flash

Результаты получены командой `pio run -d examples/size_benchmark` (или эквивалентными
`pio run -e benchmark_*` окружениями из корневого `platformio.ini`) на
`genericCH32V003F4P6`, PlatformIO `ch32v 1.1.0`, NoneOS SDK и release-сборке.
Минимальный пример намеренно не использует `printf`, UART и сканер; он сохраняет
в линковке `i2c_init`, `i2c_write_register`, `i2c_read_register` и `i2c_deinit`.
...
двух последовательных `BERR`/`ARLO`. Наибольшую экономию даёт отключение GPIO bus recovery. В этом benchmark buffer
API не вызывается, поэтому `--gc-sections` почти полностью исключает его и в
Full-профиле; если приложение вызывает `i2c_read_buffer()`/`i2c_write_buffer()`,
эффект `I2C_DISABLE_BUFFER_API` будет существенно больше, но эти API станут
недоступны.

Для приложений с регистровым вводом-выводом (датчики, EEPROM, ЦАП) Lite-режим
высвобождает около 0.5 KiB Flash. Запустить замер на своей версии SDK/компилятора
можно из `examples/size_benchmark`.

### Совместимость

Пример `examples/i2c_scanner` требует `i2c_probe_address` и **не собирается** в Lite-режиме — это сознательное поведение: пользователь получает осмысленную ошибку компиляции/линковки (`undefined reference to 'i2c_probe_address'`), а не молчаливое урезание функциональности.
