...
В версии 5.5.0 устранены малейшие уязвимости и потенциальные проблемы долгосрочной стабильности:
...
#### Преимущество Версии 5.5.0 для ЦАП:
...
```ini
build_flags =
    -DI2C_DISABLE_BUS_RECOVERY    ; убрать i2c_bus_recovery() (~65 строк)
    -DI2C_DISABLE_SCANNER          ; убрать i2c_probe_address() (~42 строки)
    -DI2C_DISABLE_BUFFER_API      ; убрать i2c_write_buffer()/i2c_read_buffer() (~110 строк)
...
Результаты получены командой `pio run -d examples/size_benchmark` на
