# Надёжный драйвер шины I2C (I2C1) для CH32V003 (RISC-V) — Версия 5.5.0 (Аудит I2C)
...
### Оценка экономии Flash

Замерено на тестовом проекте, использующем только `i2c_init` + `i2c_write_register` + `i2c_read_register` + `i2c_deinit` (флаг компиляции `-Os`, оптимизация по размеру):

| Режим | Flash | RAM |
|---|---|---|
| Full (`I2C_LITE=0`) | 3364 байт | 456 байт |
| Lite (`I2C_LITE=1`) | 2852 байт | 456 байт |
| **Экономия** | **~512 байт (~15%)** | 0 |

Для приложений, использующих только регистровый ввод-вывод (датчики, EEPROM, ЦАП), Lite-режим даёт заметное освобождение Flash под пользовательский код.
