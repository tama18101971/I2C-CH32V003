# Промышленный драйвер шины I2C (I2C1) для CH32V003 (RISC-V) — Версия 5.0 (Финал)
...
## 🛠 Архитектурные улучшения (По результатам глубокого аундаментального ревью)

В версии 5.0 устранены малейшие уязвимости и потенциальные проблемы долгосрочной стабильности (Long-Term Stability):
...
2. **Мгновенная реакция на зависания (Instant Timeout Recovery):** Логика обработки сбоев разделена на два независимых контура:
   ...
   * **Аппаратные сбои (`BERR`, `ARLO`):** Для спорадических аппаратных ошибок (ошибка шины, потеря арбитража) оставлен накопительный счетчик `consecutive_errors` с лимитом `MAX_ERROR_COUNT = 5`.
...
* `void i2c_init(uint32_t bound);`  
  Выполняет сброс блока I2C1 через `SWRST`, настраивает тактирование GPIO и периферии, вычисляет значения регистров `CTLR2`, `CKCFGR`, `TRISE` для работы в Standard Mode (до 100 кГц) или Fast Mode (до 400 кГц). Автоматически выставляет обязательный 14-й бит в `OADDR1`.
        ...
        for (uint8_t j = 0; j < 16; j++) {
            ...
            if (i2c_start() == I2C_OK) {
                if (i2c_send_addr(addr, I2C_DIR_TX) == I2C_OK) {
                    ...
                    printf("-- "); // Устройство ответило NACK
                }
                i2c_stop();
            } else {
...
#### Преимущество Версии 5.0 для ЦАП:
...
Добавьте файлы `i2c.h` и `i2c.c` в ваш проект (например, в каталог `User/` или `Hardware/`).

В файле `i2c.h` проверьте макрос `PCLK1_HZ`. Он должен строго соответствовать текущей частоте тактирования ядра микроконтроллера (по умолчанию выставлено `48000000UL` для максимальной производительности CH32V003).

Подключите заголовочный файл в кодовой базе:

```c
#include "i2c.h"
```

В функции `main()` перед началом работы с датчиками инициализируйте шину:

```c
i2c_init(400000); // Инициализация на частоте 400 кГц Fast Mode
```
